...
PieParseResult pie_feature_continue_parse_stmt(PieParseContext *ctx,
                                               PieProgram *program) {
  const PieParserApi *api = ctx->api;
  PieParser *parser = ctx->parser;

  if (!api->match(parser, PIE_TOK_CONTINUE)) {
  ...
  if (next != PIE_TOK_NEWLINE && next != PIE_TOK_COMMA && next != PIE_TOK_END &&
    ...
    api->error_at(parser, api->peek(parser), "unexpected token after continue");
    free(label_name);
    return PIE_PARSE_ERROR;
  }
