...
static PieProgram *new_body(PieParseContext *ctx) {
  ...
  if (!program) {
    pie_diag_error(ctx->api->diag(ctx->parser),
                   "out of memory while parsing anonymous block");
    return NULL;
  }
...
PieParseResult pie_feature_block_parse_stmt(PieParseContext *ctx,
                                            PieProgram *program) {
  const PieParserApi *api = ctx->api;
  PieParser *parser = ctx->parser;
  ...
  PieTokenKind next1 = api->peek_n(parser, 1)->kind;
  if (next1 == PIE_TOK_IDENTIFIER) {
    PieTokenKind next2 = api->peek_n(parser, 2)->kind;
    if (next2 == PIE_TOK_WHILE || next2 == PIE_TOK_FOR || next2 == PIE_TOK_DO) {
      return PIE_PARSE_NO_MATCH;
    }
  }
  ...
  if (!api->expect(parser, PIE_TOK_END,
                   "expected 'end' after anonymous block")) {
    free_body(body);
    return PIE_PARSE_ERROR;
  }
