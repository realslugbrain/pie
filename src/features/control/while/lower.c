...
PieLowerResult pie_feature_while_lower_stmt(PieLowerContext *ctx,
                                            const PieStmt *stmt) {
  ...
  if (!body) {
    ...
    ctx->api->error(ctx->lower, "out of memory while lowering while body");
    return PIE_LOWER_ERROR;
  }
  ...
  ir_stmt.kind = PIE_IR_STMT_WHILE;
