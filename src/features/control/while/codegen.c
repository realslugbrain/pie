...
PieAsmGenResult pie_feature_while_codegen_stmt(PieAsmCodegenContext *ctx,
                                               const PieIrStmt *stmt) {
  if (stmt->kind != PIE_IR_STMT_WHILE) {
  ...
  api->emit(cg, "    jmp .Lloop_continue_%d\n", continue_label);
